...
router.post('/searchShelterCity', async function(req,res){
    const term = req.body.searchShelterCity;
    // check if searchTerm parameter is invalid
    if (term.trim().length == 0) {
        res.status(400).render('error', { heading: "Shelter Finder", class: "error", message: "Search term keyword must be a non-empty string." });
        return;
    }
    if ( term.trim().length > 0 && !(/^[A-Za-z/ ]*$/.test(term)))
    {
        res.status(400).render('error', {heading: "Shelter Finder", class: "error", message: "Search term keyword must only contain Alphabets" });
        return;
    }
    // try to either render error HTML page if not found OR return the search results 
    try {
        const data = await shelter.searchShelterByCity(term.toLocaleLowerCase());
        if (data === undefined || data === null || data.length == 0) {
            res.render('shelterNotFound', {heading: "Shelter Finder", message: "We're sorry, but no results were found for '" + term + "'." });
            return;
        }

        res.render('shelterFound', { heading: "Shelter Found", shelter: data.slice(0, 20) });
    } catch (e) {
